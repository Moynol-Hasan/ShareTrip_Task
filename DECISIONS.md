...
1. Implemented retry with exponential backoff (3 attempts).
2. Prevents immediate failure and avoids spamming the server.
3. Shows an error + Retry button if all attempts fail.

Patterns used:

1. Reset page to 1 when filters change (better UX)
2. Reusable Pagination component (separation of concerns)
